...
int llvm::compileModuleWithNewPM(
...

  {
    ...
    if (!RunPassNames.empty()) {
      ...
      ExitOnErr(LLVMTM.buildCodeGenPipeline(MPM, MFPM, MFAM, *OS, DwoOut ? &DwoOut->os() : nullptr,
                                  FileType, Opt, &PIC));
      ...
      if (!TargetPassConfig::willCompleteCodeGenPipeline(&StopPass)) {
        ...
        StopPass = StringRef(StopPass).split(',').first.str();
        std::tie(std::ignore, IsMachinePass) =
            LLVMTM.getPassNameFromLegacyName(StopPass);
        // if (IsMachinePass)
