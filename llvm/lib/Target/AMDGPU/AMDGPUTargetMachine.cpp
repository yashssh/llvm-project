...
/// AMDGPU Code Generator Pass Configuration Options.
struct AMDGPUCodeGenPassBuilder 
  ...
  AMDGPUCodeGenPassBuilder(AMDGPUTargetMachine &TM, 
      ...
      : CodeGenPassBuilder<AMDGPUCodeGenPassBuilder>(TM, Opt, PIC) {
...
Error AMDGPUTargetMachine::buildCodeGenPipeline(ModulePassManager &MPM,
                                     ...
                                     CodeGenFileType FileType, CGPassBuilderOption Opts,
                                     PassInstrumentationCallbacks *PIC) {
    // return make_error<StringError>("buildCodeGenPipeline don't do anything yet",
    //                                inconvertibleErrorCode());
  AMDGPUCodeGenPassBuilder AMDGPUCGPB{*this, Opts, PIC};
  AMDGPUCGPB.registerAnalyses(MFAM);
  return AMDGPUCGPB.buildPipeline(MPM, MFPM, Out, DwoOut, FileType);
}

void AMDGPUTargetMachine::registerModuleAnalyses(ModuleAnalysisManager &MAM) {
  AMDGPUCodeGenPassBuilder AMDGPUCGPB{*this};
  AMDGPUCGPB.registerModuleAnalyses(MAM);
}

void AMDGPUTargetMachine::registerFunctionAnalyses(FunctionAnalysisManager &FAM) {
  AMDGPUCodeGenPassBuilder AMDGPUCGPB{*this};
  AMDGPUCGPB.registerFunctionAnalyses(FAM);
}
